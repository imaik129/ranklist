...
const RequestCard = ({request}) => {
  console.log(request.description.text)
  console.log(request)
  return (

    <Link href = {`/request/${request.slug}`}>

    <div className = "bg-white shadow-lg rounded-lg p-0 lg:p-8  mb-5 mt-5 cursor-pointer border-200 border-2 hover:border-2 border-white hover:border-gray-800">
 

        {/* {request.title} */}
        <h1 className = "transition duration-700 text-left pl-5 mb-2 pt-2 cursor-pointer hover:text-blue-600 text-xl font-bold">
          <p> {request.name}</p>
        </h1>

        <p className = " text-sm px-5 font-light font-black">
          {request.description.text}
        </p>

        <div className = "flex content-center space-x-1 items-stretch justify-left px-5 py-4">
          <Link href = {`/request/$`}> 
            <p className = "rounded-xl text-center bg-rose-400 w-content px-3 text-xs  py-1 space-x-2 font-normal font-black hover:bg-rose-500">
          ...
          <Link href = {`/request/$`}> 
            <p className = "rounded-xl text-center bg-blue-500 w-content px-3 text-xs py-1 font-normal font-black hover:bg-blue-600">
              English
            </p>
          </Link>
